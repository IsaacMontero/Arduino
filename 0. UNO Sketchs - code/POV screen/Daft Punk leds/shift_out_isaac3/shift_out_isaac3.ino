...
const char charMsg[] = "321 pPjJqQyY #Scroll-Test#";
...
byte charBuffer[24];              // Scroll "window" consists of portions of current and next character
...
void loop() {
  ...
  if (thisMs - lastMs > scrollSpeed) {  // Time to scroll yet?
   ...
   byte pinValues1[7][3] =  { { B11111100 , B00000000, B00000000 },
  ...
  for (int y = 0; y < 8; y++) {
    ...
    if(y<7){ //we just have got 7 rows
      pinValues1[y][2] = sr.SetByte(bufferByte);
    }
  }
