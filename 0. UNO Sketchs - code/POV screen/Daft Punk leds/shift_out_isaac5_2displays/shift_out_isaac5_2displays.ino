...
#define clockPin 8 // yellow
#define latchPin 9 // green
#define dataPin 12 // black

#define dataPin2 13 // second display data pin

// Initialize objects
ShiftRegister17x7 sr(3, dataPin, clockPin, latchPin);
ShiftRegister17x7 sr2(3, dataPin2, clockPin, latchPin);
//Ascii_Control ascii;
...
byte charBuffer3[8];              // First Shift Register  2nd display
byte charBuffer4[8];              // Second Shift Register 2nd display
byte charBuffer5[8];              // Third Shift Register  2nd display
...
void loop() {
  ...
  byte pinValues2[7][3]  =  { { B11111100 , 0, 0 },
                              { B11111010 , 0, 0 },
                              { B11110110 , 0, 0 },
                              { B11101110 , 0, 0 },
                              { B11011110 , 0, 0 },
  ...
  for (int y = 0; y < 8; y++) {
    bool bufferByte5[8] = {};
    for (int x = 0; x < 8; x++) {
        bufferByte5[x] =  (charBuffer5[y] >> (x) & 1);
    }
    bool bufferByte4[8] = {};
    for (int x = 0; x < 8; x++) {
        bufferByte4[x] =  (charBuffer4[y] >> (x) & 1);
    }
    bool bufferByte3[8] = {};
    for (int x = 0; x < 8; x++) {
        bufferByte3[x] =  (charBuffer3[y] >> (x) & 1);
    }
    ...
    if(y < 7){ //we just have got 7 rows
      ...
      pinValues2[y][0] = pinValues2[y][0] | (sr.SetByte(bufferByte5) & B00000001);
      pinValues2[y][1] = sr.SetByte(bufferByte4);
      pinValues2[y][2] = sr.SetByte(bufferByte3);
    }
  }
  sr.setAll(pinValues);
  sr.displayAll();
  sr2.setAll(pinValues2);
  sr2.displayAll();
}
...
void fetchChar() {
  // Add next char of the string to the buffer
  int foundVal = 0;
...
void scrollDisplay() {
  ...
  for (int x = 0; x < 8; x++) {
    charBuffer5[x] = charBuffer5[x] << 1;
    if (charBuffer4[x] & 128) {
      charBuffer5[x] = charBuffer5[x] | 1;
    }
  }
  for (int x = 0; x < 8; x++) {
    charBuffer4[x] = charBuffer4[x] << 1;
    if (charBuffer3[x] & 128) {
      charBuffer4[x] = charBuffer4[x] | 1;
    }
  }
  for (int x = 0; x < 8; x++) {
    // overpass the last 7 unused columns of the 3rd register
    charBuffer2[x] = charBuffer2[x] << 7;
    charBuffer3[x] = charBuffer3[x] << 1;
    if (charBuffer2[x] & 128) {
      charBuffer3[x] = charBuffer3[x] | 1;
    }
  }
